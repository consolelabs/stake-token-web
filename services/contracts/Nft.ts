...
export class NFT {
  ...
  async getTierWeight(): Promise<TierWeight> {
    try {
      const tier1: string = await this.provider.read({
        abi: this.abi,
        method: "tier1Weight",
        args: [],
        to: this.address,
      ...
      const tier2: string = await this.provider.read({
        abi: this.abi,
        method: "tier2Weight",
        args: [],
        to: this.address,
      ...
      const tier3: string = await this.provider.read({
        abi: this.abi,
        method: "tier3Weight",
        args: [],
        to: this.address,
      ...
      const tier4: string = await this.provider.read({
        abi: this.abi,
        method: "tier4Weight",
        args: [],
        to: this.address,
      ...
      const tier5: string = await this.provider.read({
        abi: this.abi,
        method: "tier5Weight",
        args: [],
        to: this.address,
      ...
      return {
        tier1: Number(tier1[0].toString())/10,
        tier2: Number(tier2[0].toString())/10,
        tier3: Number(tier3[0].toString())/10,
        tier4: Number(tier4[0].toString())/10,
        tier5: Number(tier5[0].toString())/10
      }
    } catch (error) {
      console.error(error);
      return {
  ...
  async listItemNft(): Promise<Attribute[]> {
    try {
      let items: Attribute[] = [];
      ...
      for (
        let id = Number(minItemId.toString());
        id <= Number(maxItemId.toString());
        id++
      ) {
        const item = await this.provider.read({
          abi: this.abi,
        ...
        if (item) {
          const contents = (await axios.get(baseUri + id.toString())).data;
          items.push({
            id: item[0].id,
            tier: item[0].tier,
            rarity: item[0].rarity,
            quantity: item[0].quantity,
            duration: Number(item[0].duration.toString()),
            boostStaking: item[0].boostStaking,
            icon: contents.image,
